      ...
      <div class="bg-white shadow p-2.5 pb-0.8 rounded-lg">
        ...
        <form action="/" class="flex md:flex-row flex-col">
          <label class="flex flex-col relative flex-grow">
            <span class="text-sm text-black-80 absolute -top-1.8">Исходная ссылка</span>
            <input value="https://www.youtube.com/watch?v=dQw4w9WgXcQ" class="input">
          </label>
          <button type="submit" class="btn md:ml-2.5 lg:mr-1.6 md:mt-0 mt-1">Сократить</button>
        </form>
        <div class="flex flex-col pt-2" v-if="items.length">
          <CutLinkItem v-for="item in items" :key="item.id" :item="item" @handleCopy='handleCopy'/>
        </div>
      </div>
    </div>
  </div>
...
import NotificationCopy from '@/components/NotificationCopy.vue'
